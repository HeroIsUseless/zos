<div align="center">
  <a href="">
    <img width="160" heigth="160" src="http://xopenbeta.com/data/app/TideReader/icon/TideReader.png">
  </a>

  <h1>ZLANG & ZOS</h1>

  <b>用自制编程语言（ZLANG）写自制操作系统（ZOS）</b><br/>
  <i>真正适合操作系统的编程语言，真正适合程序员的操作系统</i><br/>
</div>
...
- [x] 提Issues
- [x] 提Pr
- [x] 经济或技术上的援助（服务器，网站之类的）

## 介绍
zlang的基本单位为zos的基本单位，单元存储不是文件，而是基本变量，文件夹称为包。包和文件夹还是有区别的，所有的程序都是代码(函数)，所有的代码都用解释器执行。有一个存储管理器，存储管理器内，双击基础变量出现预览，双击包进入包里面，双击函数，执行函数而不进去，这只是存储管理器的特性。尽量向普通操作系统靠拢，会通过后缀名采用不同的默认程序，但后缀名的重要性被大大削弱了，只是作为名字的一部分。
