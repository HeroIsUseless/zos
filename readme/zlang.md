...
if(a, b, c)
如果a为true，则返回b，否则返回c
如果b为一系列语句，那么返回最后一个语句执行的结果，例如
if(a, (d<=a+b, b-c), (a, b, c+d))
如果a为true，则返回b-c的值，否则返回c+d的值
* 变量与函数与类统一，初始化时必赋值，没有类型系统
* a!是一个变量，a?是一个没有类型的变量


1. If与三元表达式统一，循环逻辑同理
if(a, b, (c, a+b, d))，也可以用return提前中断
2. 常量val1，变量val1!，可变类型变量val1?
3. 函数是表达式本身，触发其执行的方式是调用
