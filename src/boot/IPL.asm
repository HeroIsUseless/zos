...
; 初始化
		...
		MOV		DS,ax		    
; 设置参数
		MOV		AX,0x0820       ; 因为这是16位的，段地址是高一位的
		MOV		ES,AX			; 0x200 = 512字节，这里给了ES了把地址
		MOV		CH,0			; 柱面0
		MOV		DH,0			; 磁头0
		MOV		CL,2			; 扇区2
...
retry:
		...
		MOV		DL,0x00			; A驱动器
		INT		0x13			; 这个指令就是重置驱动器
		JMP		retry           ; 从头开始尝试
next:
		MOV		AX,ES			; 因为没有ADD ES,0x020，只能通过AX进行
		ADD		AX,0x20         ; 加上512字节，应该没错，它就是加20
		...
		MOV		DH,0
		ADD		CH,1
		CMP		CH,10 ; 读80个柱面会出错，因为16位系统只能支配1MB的内存
		; 之后最多76,777字节，而总字节数有36,330,000
		JB		readloop		; CH < CYLS 跳转到readloop

; 读取完毕，跳转到boot执行
		MOV		[0x0ff0],CH		;1 记下IPL读到哪里了
		;jmp success
		;call fin
		JMP		0x8200
